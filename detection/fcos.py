...
class DetectorBackboneWithFPN(nn.Module):
    ...
    NOTE: We could use any convolutional network architecture that
    progressively downsamples the input image and couple it with FPN.
    We use a small enough backbone that can work with Colab GPU
    and get decent enough performance.
    """
        ...
        self.fpn_params = nn.ModuleDict(
            {
                ...
                "c4->p4": nn.Conv2d(
                    in_channels=160, out_channels=self.out_channels, kernel_size=1
                ),
                "c5->p5": nn.Conv2d(
                    in_channels=400, out_channels=self.out_channels, kernel_size=1
                ),
                "p3_output": nn.Conv2d(
    ...
    def forward(self, images: torch.Tensor):
        ...
        p3 = self.fpn_params["c3->p3"](backbone_feats["c3"])
        p4 = self.fpn_params["c4->p4"](backbone_feats["c4"])
        p5 = self.fpn_params["c5->p5"](backbone_feats["c5"])
        fpn_feats["p5"] = self.fpn_params["p5_output"](p5)
        p5_upsampled = F.interpolate(fpn_feats["p5"], scale_factor=2)
        fpn_feats["p4"] = self.fpn_params["p4_output"](p4) + p5_upsampled
        p4_upsampled = F.interpolate(fpn_feats["p4"], scale_factor=2)
        fpn_feats["p3"] = self.fpn_params["p3_output"](p3) + p4_upsampled
        return fpn_feats
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        arr = torch.arange(feat_shape[-1] ** 2, dtype=torch.int)
        stride = strides_per_fpn_level[level_name]
        location_coords[level_name] = torch.zeros(feat_shape[-1] * feat_shape[-2], 2)
        location_coords[level_name][:, 0] = ((arr % feat_shape[-1]) + 0.5) * stride
        location_coords[level_name][:, 1] = ((arr // feat_shape[-1]) + 0.5) * stride
        location_coords[level_name] = location_coords[level_name].to(device).to(dtype)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(self, num_classes: int, in_channels: int, stem_channels: list[int]):
        ...
        self.stem_cls.apply(init_weights)
        self.stem_box.apply(init_weights)
        ...
        self.pred_box = nn.Conv2d(
            in_channels=stem_channels[1], out_channels=4, kernel_size=3, padding=1
        )

        # Centersness conv
        self.pred_ctr = nn.Conv2d(
            in_channels=stem_channels[0], out_channels=1, kernel_size=3, padding=1
        )
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        boxreg_deltas = {}
        centerness_logits = {}
        for layer in feats_per_fpn_level.keys():
            class_logits[layer] = torch.flatten(
                torch.permute(
                    self.pred_cls(self.stem_cls(feats_per_fpn_level[layer])),
                    (0, 2, 3, 1),
                ),
                1,
                2,
            )
            boxreg_deltas[layer] = torch.flatten(
                torch.permute(
                    self.pred_box(self.stem_box(feats_per_fpn_level[layer])),
                    (0, 2, 3, 1),
                ),
                1,
                2,
            )
            centerness_logits[layer] = torch.flatten(
                torch.permute(
                    self.pred_ctr(self.stem_box(feats_per_fpn_level[layer])),
                    (0, 2, 3, 1),
                ),
                1,
                2,
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.empty(locations.shape[0], 4)
    ...
    backgrounds = (gt_boxes[:, 0] == -1).nonzero()
    deltas[backgrounds, :] = -1
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace this with your code
    output_boxes = torch.empty(locations.shape[0], 4)
    output_boxes[:, 0] = (deltas[:, 0] * (-stride)) + locations[:, 0]
    output_boxes[:, 1] = (deltas[:, 1] * (-stride)) + locations[:, 1]
    output_boxes[:, 2] = (deltas[:, 2] * stride) + locations[:, 0]
    output_boxes[:, 3] = (deltas[:, 3] * stride) + locations[:, 1]
    backgrounds = (deltas[:, 0] == -1).nonzero()
    output_boxes[backgrounds, :] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    num = torch.minimum(deltas[:, 0], deltas[:, 2]) * torch.minimum(
        deltas[:, 1], deltas[:, 3]
    )
    den = torch.maximum(deltas[:, 0], deltas[:, 2]) * torch.maximum(
        deltas[:, 1], deltas[:, 3]
    )
    backgrounds = (deltas[:, 0] == -1).nonzero()
    centerness = torch.sqrt(num / den)
    centerness[backgrounds] = -1
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while current_index < keep.numel() - 1:
        cur_box = boxes[keep[current_index]]
        remaining_boxes = boxes[keep[current_index + 1 :]]

        iou = box_iou(cur_box[None, :], remaining_boxes)

        keep = torch.hstack(
            [
                keep[: current_index + 1],
                keep[current_index + 1 :][iou.flatten() <= iou_threshold],
            ]
        )
        current_index += 1
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        backbone_forward = self.backbone(images)
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        shapes_per_fpn_layer = {
            "p3": backbone_forward["p3"].shape,
            "p4": backbone_forward["p4"].shape,
            "p5": backbone_forward["p5"].shape,
        }
        strides_per_fpn_layer = {"p3": 8, "p4": 16, "p5": 32}
        locations_per_fpn_level = get_fpn_location_coords(
            shapes_per_fpn_layer, strides_per_fpn_layer, device=torch.device("cuda:0")
        )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_boxes = [
            fcos_match_locations_to_gt(
                locations_per_fpn_level, strides_per_fpn_layer, gt_boxes=gt_boxes[i, :]
            )
        ...
        matched_gt_deltas = [
            {
                key: fcos_get_deltas_from_locations(
                    locations_per_fpn_level[key],
                    matched_gt_boxes[i][key],
                    strides_per_fpn_layer[key],
                )
                for key in strides_per_fpn_layer.keys()
            }
        ...
        matched_gt_boxes = self._cat_across_fpn_levels(matched_gt_boxes).to(device)
        matched_gt_deltas = self._cat_across_fpn_levels(matched_gt_deltas).to(device)
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        ctr_target = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4))
        cls_targets = (
            (
                F.one_hot(
                    (matched_gt_boxes[:, :, 4].view(-1).long() + 1),
        ...
        loss_cls, loss_box, loss_ctr = (
            sigmoid_focal_loss(
                inputs=pred_cls_logits.view(-1, self.num_classes), targets=cls_targets
            ),
            0.25
            * F.l1_loss(
                input=pred_boxreg_deltas, target=matched_gt_deltas, reduction="none"
            ),
            F.binary_cross_entropy_with_logits(
                input=pred_ctr_logits.view(-1), target=ctr_target, reduction="none"
            ),
        )
        loss_box[matched_gt_deltas < 0] *= 0
        loss_ctr[ctr_target < 0] *= 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 2:
            # Replace "pass" statement with your code
            confident_detections = (level_pred_scores > test_score_thresh).to(device)

            level_pred_classes = level_pred_classes[confident_detections]
            level_pred_scores = level_pred_scores[confident_detections]
            level_deltas = level_deltas[confident_detections]
            level_locations = level_locations[confident_detections]
            # print(level_locations.shape, level_deltas.shape, level_pred_classes.shape, level_pred_scores.shape)
            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(
                level_deltas, level_locations, strides_per_fpn_layer[level_name]
            ).to(device)
            ...
            # Replace "pass" statement with your code
            level_pred_boxes = torch.clamp(
                level_pred_boxes, min=0, max=images.shape[-1]
            )
